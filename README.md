...
94 tools across 15 domains, organized by risk level:
...
| Write | 39 | `create_assignment`, `grade_submission`, `grade_with_rubric`, `comment_on_google_doc`, `edit_google_doc_text`, `update_google_slide_text`, `create_google_slides`, `randomly_assign_peer_reviews`, `upload_course_file` |
...
- **Editing Google Docs directly.** `edit_google_doc_text` replaces one quoted passage with new text (loose matching on whitespace, smart quotes, and case; `occurrence` picks a repeat; empty replacement deletes), `insert_text_in_google_doc` appends a new paragraph at the end or inserts before/after a quoted anchor, and `replace_text_in_google_doc` is a document-wide find-and-replace. These are direct edits made as the connected account — the Docs API has no suggesting mode, so they are not tracked changes — and need Editor access. Feedback on student writing should still go through `comment_on_google_doc`; the edit tools are for the instructor's own documents (syllabi, handouts, templates) or when an edit to a student doc is explicitly wanted.
